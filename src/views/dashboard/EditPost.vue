    ...
    <aside :class="[
      ...
      <nav class="flex-1 px-3 flex flex-col gap-1">
        ...
        <router-link to="/my-posts"
          class="flex items-center gap-3 px-4 py-2.5 rounded-lg bg-primary text-white font-medium text-sm shadow-sm">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <header class="flex items-center justify-between px-6 h-16 bg-white border-b border-gray-100 shrink-0 sticky top-0 z-20 gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-base font-semibold text-gray-900 leading-tight">Edit Post</h1>
            <p class="text-xs text-gray-400">Modify and update your published blog post</p>
          </div>
        </div>
      ...
      <main class="flex-1 overflow-y-auto px-6 py-6">
        <form @submit.prevent="handleSubmit" enctype="multipart/form-data">
          <div class="max-w-3xl mx-auto flex flex-col gap-6">
            ...
            <!-- Skeleton Loader for Fetching Post -->
            <div v-if="postLoading" class="space-y-6 animate-pulse">
...
const fetchMetaAndPost = async () => {
  ...
  try {
    ...
    if (blogRes?.data?.success && blogRes.data.data?.length) {
      list = blogRes.data.data
    }

    const found = list.find(b => String(b.id) === String(postId))
    if (found) {
      // Validate owner: only owner can edit
      const currentUserId = authStore.user?.user_id || authStore.user?.id
      if (found.user_id && currentUserId && found.user_id !== currentUserId) {
        toastStore.error('Unauthorized: You are not the owner of this post.')
      ...
      selectedTags.value = (found.tags || []).map(t => t.id)
      if (found.image) {
        imagePreview.value = resolveBlogImageUrl(found.image)
      }
    } else {
    ...
    toastStore.error('Failed to load post details.')
    router.push('/dashboard')
  } finally {
